 ...
 * @author amine
 */
public class CommentListForm extends BaseForm{
    Form current;

    public CommentListForm(Resources res, int id_post){
     super("Newsfeed",BoxLayout.y()); //herigate men Newsfeed w l formulaire vertical
        ...
        RadioButton mesListes = RadioButton.createToggle("NewsfeedFprm", barGroup);
        mesListes.setUIID("SelectBar");
        RadioButton liste = RadioButton.createToggle("Others", barGroup);
        liste.setUIID("SelectBar");
        RadioButton partage = RadioButton.createToggle("Comment", barGroup);
...
new NewsfeedForm(res).show();
       ...
       ArrayList<Comment> list = ServiceComment.getInstance().displayComment(id_post);

        for(Comment cat: list){
    ...
    private void addButton(Image img, Comment cat, Resources res) {    
        int height = Display.getInstance().convertToPixels(11.5f);
        int width = Display.getInstance().convertToPixels(14f);
        Button image = new Button(img.fill(width, height));
        image.setUIID("Label");
        Container cnt = BorderLayout.west(image);
        TextArea ta = new TextArea(cat.getComment());
        ta.setUIID("NewsTopLine");
        ta.setEditable(false);
        Label categorynameTxt = new Label("comment : "+cat.getComment(),"NewsTopLine2" );
...
//        cnt.add(BorderLayout.CENTER,BoxLayout.encloseY(BoxLayout.encloseX(categorynameTxt,lSupprimer)));
        ...
        lSupprimer.addPointerPressedListener(l -> {
            ...
            if(dig.show("Delete","Do You Want To Delete This Comment","Cancel","Yes")){
                dig.dispose();
            }else {
                dig.dispose();
                
                //CALLING THE DELETE METHOD FROM SERVICECATEGRY
                if(ServiceComment.getInstance().deleteComment(cat.getId_comment())) {
        ...
        lModifer.setTextPosition(LEFT);
         ...
         lModifer.addPointerPressedListener(l -> {
             ...
             new UpdateCommentNewsfeedForm(res,cat).show();
        });
